...
# https://github.com/civetweb/civetweb/issues/861, so removing prometheus from the test
  ...
  # This command updates the opentelemetry-cpp third_party_release file
  echo -e "# Copyright The OpenTelemetry Authors" > third_party_release
  echo -e "# SPDX-License-Identifier: Apache-2.0\n" >> third_party_release
  echo -e "# Git tags of third-party dependencies" >> third_party_release
  echo -e "# This file is generated by running \`./ci/do_ci.sh third_party.tags\`\n" >> third_party_release
  echo "abseil=20220623.2" >> third_party_release
  echo "zlib=v1.3.1" >> third_party_release
  echo "curl=curl-8_12_0" >> third_party_release
  echo "protobuf=v3.21.6" >> third_party_release
  echo "grpc=v1.49.2" >> third_party_release
  git submodule foreach --quiet 'echo "$name=$(git describe --tags HEAD)"' | sed -e 's:.*/::' -e 's/-[0-9]\+-g[0-9a-f]\+//' >> third_party_release
  exit 0
fi
