    ...
    const opentelemetry::trace::StartSpanOptions &options) noexcept
{
  ...
  if (!sampling_result.IsRecording())
  {
    ...
    auto non_recording_span = nostd::shared_ptr<opentelemetry::trace::Span>{
        std::make_shared<opentelemetry::trace::NoopSpan>(std::move(this->shared_from_this()),
  ...
  auto span = nostd::shared_ptr<opentelemetry::trace::Span>{
      std::make_shared<opentelemetry::sdk::trace::Span>(this->shared_from_this(), name, attributes, links, options,
