...
std::unique_ptr<trace_sdk::Sampler> BuildRuleSampler(
    std::unique_ptr<config_sdk::ComposableRuleBasedSamplerRuleConfiguration> rule)
{
  ...
  return config_sdk::TraceBuilderUtils::CreateSampler(registry.get(), sampler_config);
}

}  // namespace

TEST(TraceBuilders, InvalidBuilderContextMissingRegistry)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  auto resource = opentelemetry::sdk::resource::Resource::Create({});
  config_sdk::TracerProviderBuilderContext context{nullptr, &resource, nullptr};
  auto model = std::make_unique<config_sdk::TracerProviderConfiguration>();
  EXPECT_THROW(registry->GetTracerProviderBuilder()->Build(context, model.get()),
               config_sdk::UnsupportedException);
}

TEST(TraceBuilders, InvalidBuilderContextMissingResource)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  config_sdk::TracerProviderBuilderContext context{registry.get(), nullptr, nullptr};
  auto model = std::make_unique<config_sdk::TracerProviderConfiguration>();
  EXPECT_THROW(registry->GetTracerProviderBuilder()->Build(context, model.get()),
               config_sdk::UnsupportedException);
}
...
TEST(TraceBuilders, EmptyRegistryHasNoTraceBuilders)
{
  ...
  EXPECT_EQ(registry->GetTracerProviderBuilder(), nullptr);
}
...
TEST(TraceBuilders, SpanLimitsDefaults)
{
  ...
  auto resource = opentelemetry::sdk::resource::Resource::Create({});
  config_sdk::TracerProviderBuilderContext context{registry.get(), &resource, nullptr};
  auto provider = registry->GetTracerProviderBuilder()->Build(context, model.get());
...
TEST(TraceBuilders, SpanLimitsConfiguration)
{
  auto model                                  = std::make_unique<TracerProviderConfiguration>();
...
TEST(TraceBuilders, SpanLimitsFromAttributeLimits)
{
  // When no span-specific limits are set, top-level attribute_limits is the fallback.
  auto model    = std::make_unique<TracerProviderConfiguration>();
  model->limits = nullptr;

  auto top_limits                         = config_sdk::AttributeLimitsConfiguration{};
  top_limits.attribute_value_length_limit = 42;
  top_limits.attribute_count_limit        = 7;

  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());

  auto resource = opentelemetry::sdk::resource::Resource::Create({});
  config_sdk::TracerProviderBuilderContext context{registry.get(), &resource, &top_limits};
  auto provider = registry->GetTracerProviderBuilder()->Build(context, model.get());
  ASSERT_NE(provider, nullptr);

  auto limits = provider->GetSpanLimits();
  EXPECT_EQ(limits.attribute_value_length_limit, top_limits.attribute_value_length_limit);
  EXPECT_EQ(limits.attribute_count_limit, top_limits.attribute_count_limit);
}

TEST(TraceBuilders, SpanLimitsOverrideTopLevelAttributeLimits)
{
  // Span-specific limits take precedence over top-level attribute_limits.
  auto model                                  = std::make_unique<TracerProviderConfiguration>();
  model->limits                               = std::make_unique<SpanLimitsConfiguration>();
  model->limits->attribute_value_length_limit = 99;
  model->limits->attribute_count_limit        = 5;

  auto top_limits                         = config_sdk::AttributeLimitsConfiguration{};
  top_limits.attribute_value_length_limit = 999;
  top_limits.attribute_count_limit        = 50;
...
TEST(TraceBuilderUtils, BatchSpanProcessorDispatch)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetConsoleSpanBuilder(std::make_unique<config_test::NoopConsoleSpanExporterBuilder>());
  ...
  auto result = config_sdk::TraceBuilderUtils::CreateSpanProcessor(registry.get(), model);
  EXPECT_NE(result, nullptr);
}

TEST(TraceBuilderUtils, SimpleSpanProcessorDispatch)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetConsoleSpanBuilder(std::make_unique<config_test::NoopConsoleSpanExporterBuilder>());
...
TEST(TraceBuilderUtils, CreateParentBasedSampler)
{
  // parent based with no root configured should default to always on
  {
    ...
    auto sampler = config_sdk::TraceBuilderUtils::CreateSampler(registry.get(), sampler_config);
...
TEST(TraceBuilderUtils, CreateProbabilitySampler)
{
...
TEST(TraceBuilderUtils, CreateComposableAlwaysOnSampler)
{
...
TEST(TraceBuilderUtils, CreateComposableAlwaysOffSampler)
{
...
TEST(TraceBuilderUtils, CreateExtensionComposableSampler)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
...
TEST(TraceBuilderUtils, CreateUnregisteredExtensionComposableSampler)
{
  auto extension_config  = std::make_unique<config_sdk::ExtensionComposableSamplerConfiguration>();
  ...
  EXPECT_THROW(config_sdk::TraceBuilderUtils::CreateSampler(registry.get(), sampler_config),
               config_sdk::UnsupportedException);
}

TEST(TraceBuilderUtils, CreateComposableProbabilitySampler)
{
...
TEST(TraceBuilderUtils, CreateComposableParentThresholdSampler)
{
...
TEST(TraceBuilderUtils, CreateComposableParentThresholdSamplerNullRoot)
{
  auto parent_config =
      std::make_unique<config_sdk::ComposableParentThresholdSamplerConfiguration>();
...
TEST(TraceBuilderUtils, CreateComposableParentThresholdSamplerNestedDepth3)
{
...
TEST(TraceBuilderUtils, CreateComposableSamplerAtMaxDepth)
{
...
TEST(TraceBuilderUtils, CreateComposableSamplerBeyondMaxDepth)
{
...
TEST(TraceBuilderUtils, CreateComposableRuleBasedSampler)
{
  auto rule_based_config = std::make_unique<config_sdk::ComposableRuleBasedSamplerConfiguration>();
...
TEST(TraceBuilderUtils, RuleBasedPredicateAttributeValues)
{
...
TEST(TraceBuilderUtils, RuleBasedPredicateAttributePatterns)
{
...
TEST(TraceBuilderUtils, RuleBasedPredicateSpanKindAndParent)
{
  using config_sdk::ComposableRuleBasedSamplerRuleConfiguration;
  using trace_sdk::Decision;
...
TEST(TraceBuilderUtils, RuleBasedFirstMatchAndDefaults)
{
  using config_sdk::ComposableRuleBasedSamplerRuleConfiguration;
  using trace_sdk::Decision;
...
TEST(TraceBuilderUtils, RuleBasedSamplerIgnoresRuleWithNullSampler)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  ...
  auto sampler = config_sdk::TraceBuilderUtils::CreateSampler(registry.get(), model);
...
TEST(TraceBuilderUtils, UnregisteredAlwaysOnSamplerBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetAlwaysOnSamplerBuilder(nullptr);
  std::unique_ptr<config_sdk::SamplerConfiguration> model =
      std::make_unique<config_sdk::AlwaysOnSamplerConfiguration>();
  EXPECT_THROW(config_sdk::TraceBuilderUtils::CreateSampler(registry.get(), model),
               config_sdk::UnsupportedException);
}

TEST(TraceBuilderUtils, UnregisteredAlwaysOffSamplerBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetAlwaysOffSamplerBuilder(nullptr);
  std::unique_ptr<config_sdk::SamplerConfiguration> model =
      std::make_unique<config_sdk::AlwaysOffSamplerConfiguration>();
  EXPECT_THROW(config_sdk::TraceBuilderUtils::CreateSampler(registry.get(), model),
               config_sdk::UnsupportedException);
...
TEST(TraceBuilderUtils, UnregisteredProbabilitySamplerBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetProbabilitySamplerBuilder(nullptr);
  std::unique_ptr<config_sdk::SamplerConfiguration> model =
      std::make_unique<config_sdk::ProbabilitySamplerConfiguration>();
  EXPECT_THROW(config_sdk::TraceBuilderUtils::CreateSampler(registry.get(), model),
               config_sdk::UnsupportedException);
...
TEST(TraceBuilderUtils, UnregisteredParentBasedSamplerBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetParentBasedSamplerBuilder(nullptr);
  std::unique_ptr<config_sdk::SamplerConfiguration> model =
      std::make_unique<config_sdk::ParentBasedSamplerConfiguration>();
  EXPECT_THROW(config_sdk::TraceBuilderUtils::CreateSampler(registry.get(), model),
               config_sdk::UnsupportedException);
...
TEST(TraceBuilderUtils, UnregisteredJaegerRemoteSamplerBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetJaegerRemoteSamplerBuilder(nullptr);
  std::unique_ptr<config_sdk::SamplerConfiguration> model =
...
TEST(TraceBuilderUtils, UnregisteredTracerConfiguratorBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetTracerConfiguratorBuilder(nullptr);
  auto model = std::make_unique<config_sdk::TracerConfiguratorConfiguration>();
  EXPECT_THROW(config_sdk::TraceBuilderUtils::CreateTracerConfigurator(registry.get(), model),
               config_sdk::UnsupportedException);
}

TEST(TraceBuilderUtils, UnregisteredExtensionSamplerBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
...
TEST(TraceBuilderUtils, UnregisteredExtensionSpanExporterBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  ...
  EXPECT_THROW(config_sdk::TraceBuilderUtils::CreateSpanExporter(registry.get(), model),
               config_sdk::UnsupportedException);
}

TEST(TraceBuilderUtils, UnregisteredBatchSpanProcessorBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetConsoleSpanBuilder(std::make_unique<config_test::NoopConsoleSpanExporterBuilder>());
  registry->SetBatchSpanProcessorBuilder(nullptr);
  ...
  EXPECT_THROW(config_sdk::TraceBuilderUtils::CreateSpanProcessor(registry.get(), model),
               config_sdk::UnsupportedException);
}

TEST(TraceBuilderUtils, UnregisteredSimpleSpanProcessorBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetConsoleSpanBuilder(std::make_unique<config_test::NoopConsoleSpanExporterBuilder>());
  registry->SetSimpleSpanProcessorBuilder(nullptr);
...
TEST(TraceBuilderUtils, UnregisteredExtensionSpanProcessorBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
...
TEST(TraceBuilderUtils, UnregisteredCompositeSamplerBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetCompositeSamplerBuilder(nullptr);
...
TEST(TraceBuilderUtils, UnregisteredComposableAlwaysOnSamplerBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetComposableAlwaysOnSamplerBuilder(nullptr);
...
TEST(TraceBuilderUtils, UnregisteredComposableAlwaysOffSamplerBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetComposableAlwaysOffSamplerBuilder(nullptr);
...
TEST(TraceBuilderUtils, UnregisteredComposableProbabilitySamplerBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetComposableProbabilitySamplerBuilder(nullptr);
...
TEST(TraceBuilderUtils, UnregisteredComposableParentThresholdSamplerBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetComposableParentThresholdSamplerBuilder(nullptr);
...
TEST(TraceBuilderUtils, UnregisteredComposableRuleBasedSamplerBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetComposableRuleBasedSamplerBuilder(nullptr);
...
TEST(TraceBuilderUtils, UnregisteredTraceIdRatioBasedSamplerBuilder)
{
  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetTraceIdRatioBasedSamplerBuilder(nullptr);
  std::unique_ptr<config_sdk::SamplerConfiguration> model =
      std::make_unique<config_sdk::TraceIdRatioBasedSamplerConfiguration>();
  EXPECT_THROW(config_sdk::TraceBuilderUtils::CreateSampler(registry.get(), model),
               config_sdk::UnsupportedException);
...
TEST(TraceBuilderWorkflow, ConfigureInstallAndFlush)
{
  auto span_buffer = std::make_shared<config_test::SpanBuffer>();

  auto registry = std::make_shared<config_sdk::Registry>();
  config_sdk::RegisterDefaultTraceBuilders(registry.get());
  registry->SetExtensionSpanExporterBuilder(
      "recording", std::make_unique<config_test::RecordingSpanExporterBuilder>(span_buffer));

  auto exporter       = std::make_unique<config_sdk::ExtensionSpanExporterConfiguration>();
  exporter->name      = "recording";
  auto processor      = std::make_unique<config_sdk::SimpleSpanProcessorConfiguration>();
  processor->exporter = std::move(exporter);
  auto tp_config      = std::make_unique<config_sdk::TracerProviderConfiguration>();
  tp_config->processors.emplace_back(std::move(processor));
  auto model             = std::make_unique<config_sdk::Configuration>();
  model->tracer_provider = std::move(tp_config);
  ...
  trace_api::Provider::GetTracerProvider()->GetTracer("test")->StartSpan("s")->End();

  ASSERT_TRUE(sdk->tracer_provider->ForceFlush(std::chrono::milliseconds(5000)));
  EXPECT_EQ(span_buffer->size(), 1u);

  sdk->UnInstall();
}

TEST(TraceBuilderUtils, UnregisteredComposableAlwaysOnBuilderInParentThreshold)
{
